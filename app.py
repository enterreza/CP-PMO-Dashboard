...
def get_github_file():
    ...
    if not GITHUB_TOKEN or not REPO_NAME:
        st.warning("Silakan masukkan GitHub Token dan Repository Name di sidebar.")
        return None, None
...
def load_data(contents):
    """Membaca isi file JSON dari GitHub ke DataFrame"""
    if contents is None:
        return pd.DataFrame()
...
def save_data_to_github(repo, contents, df):
    """Menyimpan kembali DataFrame ke file JSON di GitHub"""
    ...
    st.success("Data berhasil disinkronisasi dengan GitHub!")
    st.rerun()
...
st.write("Aplikasi manajemen tugas yang terintegrasi langsung dengan repositori GitHub Anda.")

if not df_tasks.empty:
    # Mengonversi Progress ke numerik agar aman diolah
    df_tasks["Progress (%)"] = pd.to_numeric(df_tasks["Progress (%)"])

    # ==========================================
    # 3. TAB 1: DASHBOARD SUMMARY
    # ==========================================
    ...
    with tab_summary:
        ...
        todo_tasks = len(df_tasks[df_tasks["Status"] == "To Do"])
        ...
        with col_chart1:
            ...
            fig_status = px.pie(df_tasks, names='Status', color='Status',
        ...
        with col_chart2:
            ...
            fig_prog = px.bar(df_tasks, x='Task Name', y='Progress (%)', color='Status',
                              text='Progress (%)', labels={'Progress (%)': 'Progress (%)'})
            fig_prog.update_layout(margin=dict(t=20, b=20, l=20, r=20), height=300)
            fig_prog.update_traces(textposition='outside')
            st.plotly_chart(fig_prog, use_container_width=True)

        st.markdown("---")
        st.markdown("**All Active Tasks Data**")
        st.dataframe(df_tasks, use_container_width=True, hide_index=True)

    # ==========================================
    # 4. TAB 2: INPUT TASK BARU
    # ==========================================
    with tab_input:
        ...
        with st.form("input_form", clear_on_submit=True):
            ...
            submit_btn = st.form_submit_with_button("Add Task to Project")
            ...
            if submit_btn:
                if task_name and assigned_to:
                    ...
                    # Tambah ke DataFrame lama dan simpan ke GitHub
                    updated_df = pd.concat([df_tasks, pd.DataFrame([new_task])], ignore_index=True)
                    save_data_to_github(repo, contents, updated_df)
                else:
                    st.error("Mohon isi field 'Task Name' dan 'Assigned To' sebelum submit.")

    # ==========================================
    # 5. TAB 3: UPDATE TASK & PROGRESS
    # ==========================================
    with tab_update:
        st.subheader("Modify Task Status & Progress")
        
        # Pilih Task yang ingin di-update berdasarkan ID dan Nama
        ...
        if selected_option:
            ...
            with st.form("update_form"):
                st.info(f"Updating task: **{task_row['Task Name']}** (Assigned to: {task_row['Assigned To']})")
                
                # Default values diambil dari data lama
                ...
                # Otomatisasi logika status jika progress digeser ke 100 atau sebaliknya
                if new_progress == 100:
                    new_status = "Done"
                ...
                if update_btn:
                    # Cari indeks baris yang di-update dan ubah nilainya
    ...
    st.info("💡 Belum ada data project terdeteksi atau token GitHub Anda belum diisi di sidebar kanan.")
